const VER_STG_COMMON = "0.1.2"; // バージョン更新（アイテムにボム追加、ステージ間でのパワーとボムの引継ぎ、HPの全快処理）
...
class Player {
    constructor(charData) {
        ...
        // ★修正：ステージごとにHPを最大値に全回復させる（リセット）
        this.hp = this.maxHp; 
        
        this.invincibleTimer = 0; 
        
        // ★修正：パワーレベル(pw)とボムの数をステージ間で引き継ぐためのグローバル管理
        if (!window.globalPlayerState) {
            window.globalPlayerState = { powerLevel: 0, bombs: 3 };
        }
        this._powerLevel = window.globalPlayerState.powerLevel;
        this._bombs = window.globalPlayerState.bombs;
    }

    // ★追加：ゲッターとセッターを使って、値が変更されたら常にグローバル状態も更新して引き継ぐ
    get powerLevel() { return this._powerLevel; }
    set powerLevel(val) { this._powerLevel = val; window.globalPlayerState.powerLevel = val; }
    
    get bombs() { return this._bombs; }
    set bombs(val) { this._bombs = val; window.globalPlayerState.bombs = val; }
...
class Item {
    ...
    draw(ctx) {
        // ★修正：ボム（青色）を追加
        if (this.type === 'power') ctx.fillStyle = '#ffaa00';
        else if (this.type === 'recover') ctx.fillStyle = '#33ff33';
        else if (this.type === 'bomb') ctx.fillStyle = '#33ccff';
        ...
        let txt = 'P';
        if (this.type === 'recover') txt = 'H';
        if (this.type === 'bomb') txt = 'B';
        ctx.fillText(txt, this.x, this.y + 6); 
