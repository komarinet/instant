const VER_STG_KAGAMI = "0.4.0"; // バージョン更新（椎名選択時のみザコ敵の出現数を2倍に増加）
...
window.StageConfigs['kagami'] = {
    ...
    updateWaves: function(stg, timer, sW, sH) {
        ...
        if (timer > 100 && timer < 1500) {
            if (timer % 60 === 0) {
                for (let i = 0; i < multiplier; i++) stg.enemies.push(new Enemy('typea', Math.random() * sW, -50 - i*30, stg.player.charData, stg.advManager, stg.stgId));
            }
            if (timer === 500 || timer === 1000) {
                for (let i = 0; i < 3; i++) {
                    stg.enemies.push(new Enemy('typea', sW * 0.25 + i * sW * 0.25, -50, stg.player.charData, stg.advManager, stg.stgId));
                    if (isShiina) {
                        // 倍の場合は少し後ろに追加
                        stg.enemies.push(new Enemy('typea', sW * 0.25 + i * sW * 0.25, -100, stg.player.charData, stg.advManager, stg.stgId));
                    }
                }
            }
        }
        ...
        if (timer > 1500 && timer < 3000) {
            if (timer % 80 === 0) {
                for (let i = 0; i < multiplier; i++) stg.enemies.push(new Enemy('typeb', sW * 0.1 + Math.random() * sW * 0.8, -50 - i*30, stg.player.charData, stg.advManager, stg.stgId));
            }
            if (timer % 150 === 0) {
                for (let i = 0; i < multiplier; i++) stg.enemies.push(new Enemy('typea', Math.random() * sW, -50 - i*30, stg.player.charData, stg.advManager, stg.stgId));
            }
        ...
        if (timer > 3000 && timer < 4500) {
            if (timer % 90 === 0) {
                for (let i = 0; i < multiplier; i++) stg.enemies.push(new Enemy('typec', sW * 0.2 + Math.random() * sW * 0.6, -50 - i*30, stg.player.charData, stg.advManager, stg.stgId));
            }
            if (timer % 120 === 0) {
                for (let i = 0; i < multiplier; i++) stg.enemies.push(new Enemy('typeb', Math.random() * sW, -50 - i*30, stg.player.charData, stg.advManager, stg.stgId));
            }
            if (timer % 200 === 0) {
                for (let i = 0; i < multiplier; i++) stg.enemies.push(new Enemy('typea', Math.random() * sW, -50 - i*30, stg.player.charData, stg.advManager, stg.stgId));
            }
