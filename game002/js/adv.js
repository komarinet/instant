...
class ADVManager {
    ...
    draw(ctx, canvas) {
        ...
        // 1333x1500の巨大画像を毎フレーム処理して真っ黒にフリーズするのを防ぐため、一度だけ実行します
        if (!this.cachedSilhouettes) {
            ...
            this.cachedSilhouettes = preloadSilhouettes();
        }
        const silhouettes = this.cachedSilhouettes;

        // 立ち絵描画（解像度対策と二人表示レイアウト）
        if (currentMsg.character) {
            const charImg = this.assets[currentMsg.character];
            if (charImg) {
                const spriteWidth = 333; 
    ...
    drawCyberMargin(ctx, cssWidth, cssHeight, gameX, gameY, gameWidth, gameHeight, placeText, timeText, visualAreaHeight) {
        ...
        // ★修正3：Loc/Timeが画面外にはみ出さないよう、ビジュアル画面右上の安全な位置に配置★
        if (placeText || timeText) {
            // 文字が見えやすいように薄い黒帯を敷く
            ctx.fillStyle = 'rgba(0, 0, 0, 0.6)';
            ctx.fillRect(gameX + gameWidth - 140, gameY + 10, 140, 45); 
            ...
            if (placeText) {
                ctx.fillText(`Loc: ${placeText}`, infoX, gameY + 25);
            }
            if (timeText) {
                ctx.fillText(`Time: ${timeText}`, infoX, gameY + 40);
            }
            ctx.textAlign = 'left'; 
        }
    }
